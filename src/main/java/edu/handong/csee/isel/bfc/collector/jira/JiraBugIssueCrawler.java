...
public class JiraBugIssueCrawler {
	...
	public void getJiraBugs() throws IOException, InvalidProjectKeyException {
		...
		if (fileManager == null) {
			fileManager = new FileManager(this.path, this.domain, this.projectKey);
		}
		...
		boolean flag1 = requestSucceed(response.statusCode()); // flag1 is an indicator that checks whether a response
																// was succeeded when approached linkUrl with encoded
																// JQL1.
		...
		while (!flag1) {
			...
			boolean flag2 = requestSucceed(response.statusCode()); // flag2 is an indicator that checks whether a
																	// response was succeeded when approached linkUrl
																	// with encoded JQL2.

			boolean originalFlag2 = flag2; // originalFlag2 is same as value of flag2 before increasing period or
											// decreasing period.
			int originalStart = period.getStart(); // originalStart is same as value of start before increasing period
													// or decreasing period.
			...
			offInvalidProjectKeyChecking(); // From now on, there is no possibilities that the user may have entered
...
//		return fileManager.collectIssueKeys(); //collect and store issue keys into CSV file
	...
	public void crawling() {
		try {
			getJiraBugs();
		} catch (IOException | InvalidProjectKeyException e) {
			e.printStackTrace();
		}
	}

	public List<String> collectIssueKeys() {
		try {
			return fileManager.collectIssueKeys();
		} catch (IOException e) {
			e.printStackTrace();
		}
		return null;
